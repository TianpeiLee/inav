...
static adcDevice_t adcHardware[ADCDEV_COUNT] = {
    { .ADCx = ADC1, .rccADC = RCC_HB2(ADC1), .rccDMA = RCC_HB(DMA2), .DMAy_Channelx = ADC1_DMA_STREAM, .dmaMuxid= DMAMUX_DMAREQ_ID_ADC1,.enabled = false, .usedChannelCount = 0 },
};
...
void adcHardwareInit(drv_adc_config_t *init)
{
    ...
    for (int i = ADC_CHN_1; i < ADC_CHN_COUNT; i++) {
        ...
        IOConfigGPIO(IOGetByTag(adcConfig[i].tag), IO_CONFIG(DIR_IN,GPIO_MODE_IN_AN, GPIO_SPEED_VERY_HIGH, GPIO_PULL_NONE));

        adcConfig[i].adcChannel = adcChannelByTag(adcConfig[i].tag);
        adcConfig[i].dmaIndex = adc->usedChannelCount++; 
        adcConfig[i].sampleTime = ADC_SampleTime_CyclesMode7;
