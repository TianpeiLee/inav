...
uartPort_t *serialUART(UARTDevice_e device, uint32_t baudRate, portMode_t mode, portOptions_t options)
{
    ...
    if (options & SERIAL_BIDIR) {
        ...
        if (options & SERIAL_BIDIR_PP) {
            ...
            //CH32H41x without PULL_UP&DOWN regesister
            IOConfigGPIOAF(tx,
                    (options & SERIAL_BIDIR_NOPULL) ? IOCFG_AF_PP : IOCFG_AF_PP,
                    uart->tx_af);
        }
    }
    else {
        ...
        if (mode & MODE_RX) {
            ...
            IOConfigGPIOAF(rx, IOCFG_IPU, uart->rx_af);
        }
    } 
